...
async def lifespan(app: FastAPI):
    ...
    if key:
        ...
        inline = os.environ.get("SERVICE_ACCOUNT_JSON") or ""
        if inline:
            print(f"[EventFlow] SERVICE_ACCOUNT_JSON is set but NOT parseable (len={len(inline)}, head={inline[:24]!r}, tail={inline[-16:]!r}).")
        else:
